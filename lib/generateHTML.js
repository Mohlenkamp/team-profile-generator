...
function renderUniqueSection(data) {
    ...
    switch (data.role){
        ...
        case 'Engineer': {
                    return (`<p>GitHub: <span><a href="http://github.com/${data.github}" target="_blank" rel="noopener noreferrer">${data.github}</a></span><p>
                    <br>`); break;
        }
        case 'Intern': {
...
//    console.log(data)
    for (i=0;i<arrayLength;i++){  // not using foreach, because I want to limit it to 3 cards/row
        ...
        element = data[i]  
        firstName = element.name.split(" ",1)  
        returnString = returnString + `
        <div class="card">
            <header class="card-header">
                <p class="card-header-title is-centered">
                    ${firstName} </p>
                <p class="card-header-title is-centered">
                   ${renderRoleIcon(element)} ${element.role}
                </p>
            </header>
